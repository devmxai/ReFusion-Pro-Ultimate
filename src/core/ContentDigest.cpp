...
std::string sha256_content_digest(const std::span<const std::uint8_t> bytes) {
  ...
  for (std::size_t block = 0; block < padded.size(); block += 64U) {
    ...
    for (std::size_t index = 0; index < words.size(); ++index) {
      const auto upper_e = std::rotr(e, 6) ^ std::rotr(e, 11) ^
                           std::rotr(e, 25);
      const auto temporary1 = h + upper_e + choose(e, f, g) +
                              round_constants[index] + words[index];
      const auto upper_a = std::rotr(a, 2) ^ std::rotr(a, 13) ^
                           std::rotr(a, 22);
...
bool content_digest_matches(const std::span<const std::uint8_t> bytes,
                            const std::string_view expected_digest) {
  return sha256_content_digest(bytes) == expected_digest;
}
