...
int main() {
  ...
  require(opened.snapshot.project_id.value == "prj_refusion_demo_reel_30s");
  ...
  const QString invalid_path = temporary_directory.filePath("invalid.refusion.json");
  QFile invalid_file(invalid_path);
  require(invalid_file.open(QIODevice::WriteOnly));
  require(invalid_file.write("{\"schema_version\":1}") > 0);
  invalid_file.close();
